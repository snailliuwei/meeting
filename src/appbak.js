...
function handleError (err) {
  if (err) {
	...
	console.log('connect not err');
	logger.info('connect not err' + '*****');
	logger.error('connect not err' + "====");
  }
}
...
DATABASE = 'meeting';
client.query('USE ' + DATABASE, function(error, results) {
    if(error) {
...
function handleSocket(socket) {
	...
	Network_Connections++;
	database.execQuery("UPDATE t_meet_auth set currentAuth = ? where id = 1", Network_Connections, function(result){  
	...
	// Meeting materials
	socket.on('meetDoucmentList', function (id) {
		var doucmentIdArray = new Array();
		var doucmentNameArray = new Array();
		var documentNumber = new Array();
		client.query(  
			"SELECT * FROM t_meet_document t where t.meet_id='" + id +"'",  
			function selectCb(err, results, fields) {  
				if (err) {  
					throw err;  
				}
				for(var i=0;i<results.length;i++) {
					doucmentIdArray[i] = results[i].DOC_ID;
					doucmentNameArray[i] = results[i].DOC_NAME;
					documentNumber[i] = results[i].DOC_SUM
				}
				socket.emit('meetDocumentListReturn', doucmentIdArray, doucmentNameArray, documentNumber);
			}
		);
    });
	...
	socket.on('getPerName', function (perID) {
		sqloptions.sql = "SELECT * FROM t_meet_contacts t where t.per_id = ?";
		sqloptions.args = perID;
		database.execQueryCallback(sqloptions,function(result){
			console.log(result[0].PER_NAME);
			socket.emit('getPerNameReturn', result[0].PER_NAME);
			sqloptions.sql = null;
			sqloptions.args = null;
		});
    });
	...
	// Get PNG background
	socket.on('getPNG', function (title, desc) {
		var pngArray = new Array();
		client.query(  
			"SELECT * FROM t_meet_doc_note t where t.note_doc_id='" + title +"'" + " and t.note_user='" + desc + "' order by t.note_no ",  
			function selectCb(err, results, fields) {  
				if (err) {  
					throw err;  
				}
	...
	socket.on('getNextPNG', function (title, desc, currentPage) {
		var nextPNGArray = new Array();
		client.query(  
			"SELECT * FROM t_meet_doc_note t where t.NOTE_DOC_ID='" + title +"'" + " and t.NOTE_NO= '" + currentPage + "'"
			+ " and t.note_user='" + desc + "'",
			function selectCb(err, results, fields) {  
				if (err) {  
					throw err;  
				}
	...
	socket.on('getLastPNG', function (title, desc, currentPage) {
	...
	// Synchronous
	socket.on('sync', function (title, id, desc, currentPage) {
		console.log('sync:' + id + "===" + currentPage);
		socket.broadcast.emit('syncReturn', title, id, desc, currentPage);
    });
	
	socket.on('syncnoqz', function (title, id, desc, currentPage) {
		var perName;
		client.query(  
			"SELECT * FROM t_meet_contacts t where t.per_id='" + desc + "'",  
			function selectCb(err, results, fields) {  
				if (err) {  
					throw err;  
				}
				...
				socket.broadcast.emit('syncnoqzReturn', title, id, desc, currentPage, perName);
			}
		);
	...
	socket.on('sendmessage', function (ids, content) {
		console.log('sendmessage:' + ids + "===" + content);
